...
/*             String error=new String(data);
    ...
    public Bitmap loadMapTile(Context ctx, int x, int y, int z, boolean allowDownload) {
        ...
        if ((tile == null) && (mode == MODE_GSMAP))
            tile = loadMapTile(ctx, x, y, z, allowDownload, MODE_GMAP);
        return tile;
    }
    ...
    private Bitmap loadMapTile(Context ctx, int x, int y, int z, boolean allowDownload, int useMode) {
        ...
        try {
            ...
            if (allowDownload) {
                ...
                } else if (useMode == MODE_GMAP)
                    tile = downloadTile("http://mt" + mirrorCnt + ".google.com/vt/v=w2.97&z=" + z + "&x=" + x + "&y=" + y, x, y, z, ctx, tilePath, true);
                else
                    tile = downloadTile("http://khm" + mirrorCnt + ".google.com/kh/v=99&z=" + z + "&x=" + x + "&y=" + y, x, y, z, ctx, tilePath, true);
                mirrorCnt++;
